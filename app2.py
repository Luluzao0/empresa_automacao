...
def enviar_mensagem(telefone, mensagem):
    try:
        ...
        pwk.sendwhatmsg_instantly(telefone, mensagem, wait_time=8, tab_close=True)
    ...
    if telefone_individual:
        ...
        for index, row in df.iterrows():
            ...
            if not telefone.startswith("+"):
                telefone = "+" + telefone
            st.write(f"Enviando mensagem para: {telefone}")
            resultado = enviar_mensagem(telefone, mensagem)
            st.write(f"{telefone}: {resultado}")
            time.sleep(8)  # Reduz o tempo de espera para 8 segundos
        if stop:
